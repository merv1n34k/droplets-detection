...
class DropletDetector:
    ...
    def __init__(
        ...
        min_circularity : float
            ...
            is a perfect circle)
        max_border_dist : float
            Maximum distance of centroid origin to image border
        ...
        self.max_border_dist = max_border_dist
        ...
        print(f"Initialized DropletDetector with:\n"
              ...
              f" - Max border distance: {max_border_dist} pixels\n"
    ...
    def process_image(self, image, image_name=""):
        ...
        # Check conversion_factor for correctness
        if type(self.conversion_factor) in (int, float):
            conversion_factor = self.conversion_factor
        else:
            print(f"Conversion factor \"{self.conversion_factor}\" is incorrect!")
            conversion_factor = 1
        ...
        for region in props:
            ...
            if (circularity >= self.min_circularity and
                    ...
                    diameter <= self.max_diameter and
                    region.centroid[0] >= self.max_border_dist and
                    region.centroid[0] <= (image.shape[1] - self.max_border_dist) and
                    region.centroid[1] >= self.max_border_dist and
                    region.centroid[1] <= (image.shape[0] - self.max_border_dist)):
                ...
                result = {
                    ...
                    'area_px': area,
                    'area_um2':  (area * conversion_factor),
...
def main():
    ...
    parser.add_argument('--max_border_dist', type=float, default=0,
    ...
    detector = DropletDetector(
        ...
        max_border_dist=args.max_border_dist,
